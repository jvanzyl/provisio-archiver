...
public class FileSystemAssert {
    ...
    public static void assertPresenceAndContentOf(File outputDirectory, String fileName, String expectedContent)
            throws IOException {
        File file = new File(outputDirectory, fileName);
    ...
    public static void assertFileIsHardLink(File outputDirectory, String fileName) throws Exception {
        ...
        assertTrue(
                String.format("We expect to find the file %s, but it doesn't exist or is not a hardlink.", fileName),
                file.exists() && file.isFile() && hardlink);
    }
    ...
    public static void assertFileIsNotHardLink(File outputDirectory, String fileName) throws Exception {
        ...
        boolean hardlink = Files.getAttribute(file.toPath(), "unix:nlink").equals(5);
        assertFalse(
                String.format("We expect to find the file %s, but is a hardlink and it should not be.", fileName),
                file.exists() && file.isFile() && hardlink);
    }
